...
class subscription_dbaccess
{
    public static function add($new_subscription)
    {
        ...
        try {
            ...
            $stmt->bindParam(':prd', $new_subscription->period_subscription);
    ...
    public static function list($idm)
    {
        ...
        try {
            $pdo = database_lib::generate_pdo_object();
            $stmt = $pdo->prepare('SELECT * FROM `subscription` WHERE `member_subscription` = :idm');
            /* $stmt = $pdo->prepare('SELECT s.*, g.*, p.* gm.*
                                     FROM `gym` gm, `person` p, `subscription` s, `group` g,
                                    WHERE p.`state_person`         = 1     AND 
                                          p.`state_person`         = :stt  AND  
                                          s.`member_subscription`  = p.`id_person`
                                          s.`group_subscription`   = g.`id_group`'
                                          g.`gtm_group`            = gm.`ig_gym`);*/
