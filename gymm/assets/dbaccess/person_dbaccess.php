...
class person_dbaccess
{
    ...
    public static function list_by_group($idgr)
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT DISTINCT p.* 
                                              FROM `group` g, subscription s, person p
                                             WHERE g.id_group = :idgr AND 
                                                   g.id_group = s.group_subscription AND 
                                                   s.member_subscription = p.id_person AND 
                                                   NOW() BETWEEN s.date_begining_subscription and 
                                                                 s.date_ending_subscription');
            $stmt->bindParam(':idgr', $idgr);
            $stmt->execute();
            $result = (isset($stmt) && ($stmt !== false) && ($stmt->rowCount() > 0)) ? $stmt : $result;
        }
        catch (Exception $exception) {
    ...
    public static function search($KeyWords)
    {
        ...
        try {
            $req = "SELECT * 
                      FROM `person` 
                     WHERE (`first_name_person` LIKE :kwd OR 
            ...
            $stmt = $pdo->prepare("SELECT * 
                                    ...
                                    WHERE (`first_name_person` LIKE :kwd OR 
                                           `last_name_person`  LIKE :kwd OR 
                                           `mobile_person`     LIKE :kwd OR 
                                           `email_person`      LIKE :kwd) AND `state_person` = 1");
    ...
    public static function consult_($acn)
    {
        ...
        try {
            $pdo = database_lib::generate_pdo_object();
            $stmt = $pdo->prepare('SELECT p.*, s.*
                                     FROM `person` p,
                                           subscription s
                                    WHERE p.`access_number_person` LIKE :acn AND
                                          p.id_person = s.member_subscription');
